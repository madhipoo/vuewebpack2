...
export default {
    ...
    methods:{
        getProducts(page = 1){
            const api = `${process.env.APIPATH}/api/${process.env.COSTOMPATH}/products?page=${page}`;//'https://vue-course-api.hexschool.io/api/leviathan91608/products';
            const vm = this;
            //API 伺服器路徑
            // 所申請的APIPath
        ...
        deleteProduct(){
            const vm = this;
            const api = `${process.env.APIPATH}/api/${process.env.COSTOMPATH}/admin/product/${vm.tempProduct.id}`;
            vm.isLoading = true;
